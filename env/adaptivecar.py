...
class AdaptiveCar(gym.Env):
    ...
    def reset_1(self):
        ...
        self.v_lead = self.np_random.uniform(28, 28)
        self.v_ego = self.np_random.uniform(20,40)
    ...
    def reset(self):
        if self.mode==0:
            ...
            self.x_ego = self.np_random.uniform(0, 0)
            self.x_lead = self.np_random.uniform(20, 60)
            self.delta_x = self.x_lead - self.x_ego
            self.delta_v = self.v_lead - self.v_ego
        elif self.mode==1:
            self.y_lead = self.y_ego = 0
            self.v_lead = self.np_random.uniform(28, 28)
            self.v_ego = self.np_random.uniform(31.85, 31.9)
            self.x_ego = self.np_random.uniform(0, 0)
            self.x_lead = self.np_random.uniform(3.05, 3.06)
            self.delta_x = self.x_lead - self.x_ego
            self.delta_v = self.v_lead - self.v_ego
        elif self.mode == 2:
            self.y_lead = self.y_ego = 0
            self.v_lead = self.np_random.uniform(28, 28)
            self.v_ego = self.np_random.uniform(26,32)
            self.x_ego = self.np_random.uniform(0, 0)
            self.x_lead = self.np_random.uniform(5, 8)
            self.delta_x = self.x_lead - self.x_ego
            self.delta_v = self.v_lead - self.v_ego
        elif self.mode == 3:
            self.y_lead = self.y_ego = 0
            self.v_lead = self.np_random.uniform(28, 28)
            self.v_ego = self.np_random.uniform(28,28)
            self.x_ego = self.np_random.uniform(0, 0)
            self.x_lead = self.np_random.uniform(4,4)
            self.delta_x = self.x_lead - self.x_ego
            self.delta_v = self.v_lead - self.v_ego
        if not self.simplified_mode:
            return np.array(
                [self.x_lead, self.x_ego, self.v_lead, self.v_ego, self.y_lead, self.y_ego, delta_v, delta_x])
        else:
            return np.array([self.delta_v, self.delta_x])
    ...
    def step(self, action_ego):
        if action_ego == 0:
            ...
            acceleration = self.a_ego
        self.delta_x = self.delta_x+self.dt*self.delta_v
        self.delta_v = self.delta_v-acceleration*self.dt
        #cost = 0
        done = False
        if self.cost_function_index == 0:
            cost = -(0.02 * (self.delta_x - self.d_default)) ** 2
        else:
            cost = 1
            if self.delta_x < 0:  # crash
                done = True
                cost = -1000
        return np.array([self.delta_v, self.delta_x]), cost, done, {}
